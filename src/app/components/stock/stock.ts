...
const PRODUCT_TYPES: { value: ProductType; label: string }[] = [
  { value: 'venta', label: 'Venta (pelotas, agua, gaseosa)' },
...
/**
 * Stock: listado de productos (paletas para alquilar, pelotas/agua/gaseosa
 * para vender) con aviso de stock bajo, alta/edición/baja (solo admin) y
 * venta de un producto (ambos roles, igual que cobran en Caja) suelta o
 * sumada al total de una reserva de hoy.
 */
@Component({
  selector: 'app-stock',
  standalone: true,
...
export class Stock implements OnInit {
  ...
  // Venta de producto
  readonly sellProduct = signal<Product | null>(null);
  readonly sQuantity = signal(1);
  readonly sMethod = signal<PaymentMethod>('efectivo');
  ...
  openSell(p: Product): void {
    this.sellProduct.set(p);
    this.sQuantity.set(1);
    this.sMethod.set('efectivo');
    this.sBookingId.set('');
    this.sError.set('');
    this.loadingBookings.set(true);
    this.bookingsService.getBookings(todayIso()).subscribe({
      next: list => {
  ...
  bookingLabel(b: Booking): string {
    return `${b.customerName} · ${b.courtName} · ${b.startHour}-${b.endHour}hs`;
  }

  submitSell(): void {
    const product = this.sellProduct();
    if (!product) return;
    if (this.sQuantity() <= 0) {
    ...
    this.service
      .sell({
        ...
        paymentMethod: this.sMethod(),
        ...(this.sBookingId() ? { bookingId: this.sBookingId() } : {}),
      })
      .subscribe({
        next: () => {
